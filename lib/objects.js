...
function GridHelperParams(grid_size, auto_grid_size, display)
{
    ...
    this.x_relative_dist = 0;
    this.y_relative_dist = 0;
    this.z_relative_dist = 0;

    this.update = function (XYGridHelper, YZGridHelper, ZXGridHelper)
    {
        /**
         * Update derived params from the generated grid helpers.
         *
         * Args:
         *  XYGridHelper, YZGridHelper, ZXGridHelper (Object3D): grid helpers
         *      created in MainViewer.setupGridHelper()
         */

        // sign: for grid helper position
        this.sign.x = Math.sign(YZGridHelper.position.x);
        this.sign.y = Math.sign(ZXGridHelper.position.y);
        this.sign.z = Math.sign(XYGridHelper.position.z);

        // relative distance of each grid helper from the origin
        this.x_relative_dist = Math.abs(YZGridHelper.position.x);
        this.y_relative_dist = Math.abs(ZXGridHelper.position.y);
        this.z_relative_dist = Math.abs(XYGridHelper.position.z);

        // remember grid sizes to detect changes in MainViewer.animate()
        this.previous_grid_size_x = this.grid_size_x;
        this.previous_grid_size_y = this.grid_size_y;
        this.previous_grid_size_z = this.grid_size_z;
    };
...
class MainViewer
{
    ...
    setupGridHelper()
    {
        ...
        // update ghParams from the generated grid helpers
        this.ghParams.update(
            this.XYGridHelper,
            this.YZGridHelper,
            this.ZXGridHelper
        );
